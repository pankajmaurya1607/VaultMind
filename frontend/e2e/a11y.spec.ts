...
test.describe("Accessibility (axe)", () => {
  test("landing page has no serious violations", async ({ page }) => {
    ...
    // color-contrast is expected for dark muted text (design choice) — filter it for this marketing page
    const filtered = results.violations.filter((v) => v.id !== "color-contrast" && (v.impact === "critical" || v.impact === "serious"))
  ...
  test("login page has no serious violations", async ({ page }) => {
    ...
    const filtered = results.violations.filter((v) => v.impact === "critical" && v.id !== "color-contrast")
    expect(filtered).toEqual([])
  })
  ...
  test("dashboard (authenticated) has no critical violations", async ({ page }) => {
    await page.addInitScript(() => {
      ...
      localStorage.setItem("refresh_token", "mock")
    })
