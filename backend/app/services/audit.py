...
class AuditService:
    ...
    async def log(
        self, user_id: int | None, user_email: str | None, action: str, resource: str,
        resource_id: str | None = None, details: str | None = None,
        ip_address: str | None = None, success: bool = True,
    ) -> None:
        await self.repo.create(
            user_id=user_id, user_email=user_email, action=action, resource=resource,
...
async def audit_event(
    ...
    *,
    ...
    """Best-effort audit write that cannot break the originating request."""
    try:
        service = AuditService(db)
        await service.log(
            user_id=user_id, user_email=user_email, action=action, resource=resource,
            resource_id=str(resource_id) if resource_id is not None else None,
            details=details, ip_address=service.get_client_ip(request), success=success,
        )
        await db.commit()
